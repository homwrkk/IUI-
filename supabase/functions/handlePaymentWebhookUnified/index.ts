...
serve(async (req) => {
  ...
  try {
    ...
    let webhookTableName = membershipType === 'member' ? 'members_webhook_events' : 'creators_webhook_events';
    let auditTableName = membershipType === 'member' ? 'members_payment_audit_log' : 'creators_payment_audit_log';
    ...
    if (!transaction) {
      ...
      if (otherTxData) {
        ...
        webhookTableName = membershipType === 'member' ? 'members_webhook_events' : 'creators_webhook_events';
        auditTableName = membershipType === 'member' ? 'members_payment_audit_log' : 'creators_payment_audit_log';
      }
    }
    ...
    await supabase.from(webhookTableName).insert({
