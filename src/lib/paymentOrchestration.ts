...
export class PaymentOrchestration {
  ...
  /**
   * Get the appropriate audit table name based on membership type
   */
  private static getAuditTableName(membershipType: 'creator' | 'member' = 'creator'): string {
    return membershipType === 'member' ? 'members_payment_audit_log' : 'creators_payment_audit_log';
  }

  /**
   * Get the appropriate webhook table name based on membership type
   */
  private static getWebhookTableName(membershipType: 'creator' | 'member' = 'creator'): string {
    return membershipType === 'member' ? 'members_webhook_events' : 'creators_webhook_events';
  }
  ...
  private static async validatePaymentRequest(
    ...
    if (tierOrder[request.targetTier] <= tierOrder[request.currentTier]) {
  ...
  private static async logAuditEvent(
    ...
    try {
      const auditTableName = this.getAuditTableName(membershipType);
      await supabase
        .from(auditTableName)
        .insert({
  ...
  static async handleWebhook(webhook: WebhookPayload, membershipType: 'creator' | 'member' = 'creator'): Promise<void> {
    try {
      ...
      if (webhook.status === 'completed' || webhook.status === 'success') {
        await this.completePayment(transaction.id, webhook.transactionId);
      } else if (webhook.status === 'failed') {
        await this.failPayment(transaction.id, webhook.payload.error);
      }
    } catch (error) {
  ...
  private static async recordWebhookEvent(
    ...
    try {
      const webhookTableName = this.getWebhookTableName(membershipType);
      await supabase
        .from(webhookTableName)
        .insert({
          transaction_id: transactionId,
